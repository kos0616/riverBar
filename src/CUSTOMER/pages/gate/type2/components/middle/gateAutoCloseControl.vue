...
const avaiableGates = computed(() => {
  ...
  function generateGate(n?: string) {
    ...
    const datas = PLCs.value.filter((o) =>
      ['676', '436', '313', '613'].some((str) => str + suffix === o.Tag)
    );

    const isActive = getValue(['436', '676'], datas, n) === true;
    const triggerTag = getTriggerTag(datas, n);

    const waterLevelTag = getWaterLevelTag(datas, n);
    const waterLevel = getValue(['313', '613'], datas, n) as number;
    const _name = getName('676', datas, n) || getName('436', datas, n) || `水門${n || '1'}`;
  ...
  async function triggerSwitch(
    ...
    switch (tag) {
      case '436':
        Tag = targetValue === true ? '436' + suffix : '';
        allTags = ['436' + suffix];
        // opne/close: 436
        break;
      case '676':
        Tag = targetValue === true ? '676' + suffix : '';
        allTags = ['676' + suffix];
        // open/cose: 676
        break;
      default:
...
const controlDeviceByTag = async (
  ...
  const action = targetValue === true ? '啟動自動關門' : '關閉自動關門';
  try {
...
const editDeviceNumberByTag = async (
  ...
  try {
    const Value = await ElMessageBox.prompt('請輸入自動關門水位(㎝)', device, {
      inputType: 'number',
      inputPattern: /^\d+(\.\d+)*$/,
    ...
    ElMessage.success(`自動關門水位調整為 ${Value}㎝`);
    emitter.emit('refreshIws');
  } catch (e) {
    if ((e as string) === 'cancel') {
      return;
